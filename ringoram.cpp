      ...
      cache_levels(cache_levels)
{
    ...
    init_tree_top_cache(cache_levels);
...
void ringoram::init_tree_top_cache(int t)
{
    int num_top_buckets = (1 << t) - 1;
    tree_top_cache.resize(num_top_buckets);
}
...
void ringoram::localReadBucket(int pos)
{
    bucket& bkt = tree_top_cache[pos];
...
void ringoram::localWriteBucket(int pos)
{
    int level = GetlevelFromPos(pos);
    vector<block> blocksTobucket;
...
bool ringoram::WritePathFull(int leaf_id, const std::vector<bucket>& buckets_to_write) {
    try {
        ...
        for (size_t i = L; i >= cache_levels; i--) {
...
void ringoram::EvictPath()
{
    ...
    for (i = 0; i < cache_levels; i++) {
        int pos = Path_bucket(l, i);
        if (!isPositionCached(pos)) continue;
        localReadBucket(pos);
    }

    for(i=cache_levels-1;i>=0;i--)
    {
        int pos = Path_bucket(l, i);
        if (!isPositionCached(pos)) continue;
        localWriteBucket(pos);
    }
...
void ringoram::EarlyReshuffle(int l)
{
    ...
    {
        int position=Path_bucket(l, i);
        if (tree_top_cache[position].count >= dummyBlockEachbkt)
        {
	        localReadBucket(position);
	        localWriteBucket(position);
        }
    }
    ...
    std::vector<uint8_t> request_data(sizeof(int32_t));
    *reinterpret_cast<int32_t*>(request_data.data()) = l;
